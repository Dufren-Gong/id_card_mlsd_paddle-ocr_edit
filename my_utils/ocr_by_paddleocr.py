...
def get_catch(texts, nation, catch, info_checks):
    if nation == '内地':
        if catch == None:
            catch = [None] * 6
        catch = get_infos(texts, catch, info_checks)
    elif nation == '香港':
        if catch == None:
            catch = [None, None, "无", None, "无", None]
...
def pic_to_str(image, ocr, pass_flag=False, shape = (856, 540), scale = 0.8, times = 1, pic_type='origin', info_checks = dict(), one_line_scale = 30):
    if pass_flag:
        return ['None'] * 7, ['error']
    if scale > 1.0:
        scale = 1.0
    ...
    if times == 2:
        ...
        if results != [None]:
            ...
            catch = get_catch(texts1, nation, catch, info_checks)
    if nation == None:
        nation = '内地'
    if catch != None:
        catch.append(nation)
...
def get_infos(infos, catch, info_checks):
